...
function inspectCoupangProductDetail() {
  ...
  return {
    ...
    source_image_urls: imageCandidates.filter((url) => url.includes('coupangcdn.com/') && !url.includes('/thumbnails/remote/')).slice(0, 4),
...
function approvalCandidate(detail, affiliateUrl) {
  return {
    ...
    original_image_urls: Array.isArray(detail.source_image_urls) ? detail.source_image_urls : [detail.source_image_url].filter(Boolean),
